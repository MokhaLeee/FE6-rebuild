...
void UnpackRawMap(void * buf, int chapter)
{
    ...
    Decompress(gpChapterAssets[GetChapterInfo(chapter)->asset_tileset], sTilesetInfo);
...
void ApplyChapterMapGraphics(int chapter)
{
    ...
    Decompress(
        gpChapterAssets[GetChapterInfo(chapter)->asset_img_a],
        (void *) BG_VRAM + CHR_SIZE * BGCHR_TILESET_A);

    // Decompress tileset graphics (part 2, if it exists)
    if (gpChapterAssets[GetChapterInfo(chapter)->asset_img_b])
        Decompress(
            gpChapterAssets[GetChapterInfo(chapter)->asset_img_b],
            (void *) BG_VRAM + CHR_SIZE * BGCHR_TILESET_B);

    // Apply tileset palette
    ApplyPalettes(gpChapterAssets[GetChapterInfo(chapter)->asset_pal], BGPAL_TILESET, 10);
}

void ApplyChapterMapPalettes(void)
{
    ApplyPalettes(gpChapterAssets[GetChapterInfo(gPlaySt.chapter)->asset_pal], BGPAL_TILESET, 10);
}
