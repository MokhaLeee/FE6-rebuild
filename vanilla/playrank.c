...
u8 PlayRankGetter_Tactics(void)
{
	...
	for (i = 0; i < slot; i++) {
		...
		ranks[0] += gpChapterInfoTable[chapter_stat->chapter_id].tactics_ranks[PALYRANK_A];
		ranks[1] += gpChapterInfoTable[chapter_stat->chapter_id].tactics_ranks[PALYRANK_B];
		ranks[2] += gpChapterInfoTable[chapter_stat->chapter_id].tactics_ranks[PALYRANK_C];
		ranks[3] += gpChapterInfoTable[chapter_stat->chapter_id].tactics_ranks[PALYRANK_D];
	}
...
u8 PlayRankGetter_Experience(void)
{
	 ...
	 */
	ret = PALYRANK_D;
	if (total_level > gpChapterInfoTable[chapter_id].exp_ranks[REFRANK_D]) {
		ret = PALYRANK_C;

		if (total_level > gpChapterInfoTable[chapter_id].exp_ranks[REFRANK_C]) {
			ret = PALYRANK_B;

			if (total_level > gpChapterInfoTable[chapter_id].exp_ranks[REFRANK_B]) {
				ret = PALYRANK_A;

				if (total_level > gpChapterInfoTable[chapter_id].exp_ranks[REFRANK_A])
					ret = PALYRANK_S;
			}
		}
	}
...
u8 PlayRankGetter_Power(void)
{
	 ...
	 */
	ret = PALYRANK_D;
	if (total_level > gpChapterInfoTable[chapter_id].combat_ranks[REFRANK_D]) {
		ret = PALYRANK_C;

		if (total_level > gpChapterInfoTable[chapter_id].combat_ranks[REFRANK_C]) {
			ret = PALYRANK_B;

			if (total_level > gpChapterInfoTable[chapter_id].combat_ranks[REFRANK_B]) {
				ret = PALYRANK_A;

				if (total_level > gpChapterInfoTable[chapter_id].combat_ranks[REFRANK_A])
					ret = PALYRANK_S;
			}
		}
	}
