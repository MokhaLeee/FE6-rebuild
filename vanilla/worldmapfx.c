...
void DrawWmArrowCore(u8 id, i16 b, i16 c, int d)
{
    PutSpriteAffine(
        id,
        (gSinLut_u32[(d + 0x100) & 0x3FF] * b) >> 0xF,
        (gSinLut_u32[(d) & 0x3FF] * c) >> 0xF,
        -((gSinLut_u32[(d) & 0x3FF] * b) >> 0xF),
...
{
    if (gpChapterAssets[GetChapterInfo(gPlaySt.chapter)->asset_event_info_wm])
        StartEvent(gpChapterAssets[GetChapterInfo(gPlaySt.chapter)->asset_event_info_wm]);
}
