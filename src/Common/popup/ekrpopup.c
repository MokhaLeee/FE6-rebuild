...
void DrawBattlePopup(struct ProcEkrPopup *proc, int type, u32 item)
{
	...
	LZ77UnCompVram(Img_EkrUnkPopup, (void *)BG_VRAM + BGCHR_EKRPOPUPBG * 0x20);
	...
	if (type == 0) {
		ApplyIconPalette(1, 0x10 + BGPAL_POPUPICON);
		PutIconObjImg(WTYPE_ICON(GetItemKind(item)), OBCHR_BANIM_EKRPOPUPICON);
	} else {
		ApplyIconPalette(0, 0x10 + BGPAL_POPUPICON);
		PutIconObjImg(GetItemIcon(item), OBCHR_BANIM_EKRPOPUPICON);
	}
