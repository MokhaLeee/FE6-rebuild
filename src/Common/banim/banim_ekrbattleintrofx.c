...
void EkrUnitKakudai_PrepareAnimScript(struct ProcEkrUnitKakudai *proc)
{
	...
	if (gBanimValid[POS_L] == 1) {
		struct BanimScrFrame *scr = (void *)(gpBanimScrs[POS_L] + gpBanimModesLeft[front_mode]);
	...
	if (gBanimValid[POS_R] == 1) {
		struct BanimScrFrame *scr = (void *)(gpBanimScrs[POS_R] + gpBanimModesRight[front_mode]);
		const void *src = scr->img;
