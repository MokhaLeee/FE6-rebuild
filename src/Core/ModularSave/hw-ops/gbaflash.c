...
{
	for (int i = 0; i < LOOP_CNT_PER_MILLI_SECOND * timeout && flash_absmemcmp(&flash_mem[addr], data, 1); i++);
...
int flash_erase_and_write_atmel(u32 addr, u8 *data) {
	...
	// write 128 bytes
	for (i = 0; i < FLASH_SECTOR_SIZE_128B - (addr & (FLASH_SECTOR_SIZE_128B - 1)); i++)
...
int flash_write_atmel(u32 addr, u8 *data, size_t size) {
	...
	if (addr % FLASH_SECTOR_SIZE_128B) {
		...
		size_t written = FLASH_SECTOR_SIZE_128B - addr % FLASH_SECTOR_SIZE_128B;
