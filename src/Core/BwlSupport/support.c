...
{
	int i, count = GetUnitSupportCount(unit);
	struct PidStats *bwl_other, *bwl = GetPidStats(UNIT_PID(unit));
	...
	for (i = 0; i < count; ++i) {
		...
		bwl_other = GetPidStats(UNIT_PID(other));
		if (bwl_other == NULL)
			return;

		bwl_other->supports[GetUnitSupportNumByPid(other, unit->pinfo->id)] = 0;
		bwl->supports[i] = 0;
	}
}
...
void UnitInitSupports(struct Unit *unit)
{
	...
	for (i = 0; i < count; ++i)
		bwl->supports[i] = GetUnitInitialSupportExp(unit, i);
}
...
bool DoSupportAction(ProcPtr proc)
{
	...
	struct PidStats *bwlA = GetPidStats(gActiveUnit->pinfo->id);
	struct PidStats *bwlB = GetPidStats(target->pinfo->id);
	...
	expA = bwlA->supports[numA];
	expB = bwlB->supports[numB];

	if (expA != expB) {
		if (expA > expB)
			bwlB->supports[numB] = expA;

		if (expA < expB)
			bwlA->supports[numA] = expB;
	}
