...
ROM = "FE6Localization_v1.2.1.gba"
# ROM = "EN.gba"
TEXT_TABLE = 0x833530
TEXT_COUNT = 0xD10
...
def get_pr_offset(rom_data, addr):
    ret = int.from_bytes(rom_data[addr + 0:addr + 4], 'little') & 0xFFFFFF
    logprint('DEBUG', f"[0x{addr:06X}]=0x{ret:08X}")
    return ret
...
def decode_single_string(msg_idx, rom_data, huffman_table):
    ...
    if pr_encodered_string == 0x0809FAC0:
        return NULL
...
'''
    parsed_string = decode_single_string(0x7F1, rom_data, huffman_table)
    print(parsed_string)
'''
